	...
	fastify.get<{
		...
		if (canCreate.allowed) {
			return {
				allowed: true,
				accessSource:
					canCreate.source === "invite" ? "invite" : canCreate.source === "disabled" ? "disabled" : "wallet",
			};
		}
	...
	fastify.post<{
		...
		try {
			...
			if (request.launchGate?.consumeInviteCode && user?.solana) {
				...
				try {
					await launchGateService.redeemInviteCode(inviteCodeToConsume, user.solana);
				} catch (inviteError) {
					await DB.Token.deleteOne({ _id: newToken._id });
					const message = inviteError instanceof Error ? inviteError.message : "Invite code could not be claimed";
					return reply.code(409).send({
						success: false,
						error: message,
					});

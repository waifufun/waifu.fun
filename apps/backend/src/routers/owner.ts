...
export default async function ownerRoutes(fastify: FastifyInstance) {
			...
			try {
				...
				if (!tokenData.cloudAgentId) {
					return {
						success: true,
						runtime: {
							agentStatus: tokenData.agentStatus || "none",
		...
		async (request, reply) => {
			try {
				...
				if (billingMode !== undefined) {
					if (!VALID_BILLING_MODES.includes(billingMode as (typeof VALID_BILLING_MODES)[number])) {
						return reply.code(400).send({
							success: false,
	...
	fastify.post("/tokens/:chain/:chainId/:mint/claim", async (request, reply) => {
		try {
			...
			const walletAddresses = [user.evm, user.solana].filter((address): address is string => Boolean(address));
			...
			const matchedCreatorAddress = walletAddresses.find((address) => address === tokenData.creator);
			if (!tokenData.creator || !matchedCreatorAddress) {
			...
			const userData = await DB.User.findOne({ address: matchedCreatorAddress }).lean();
			if (!userData?._id) {
				return reply.code(404).send({
					success: false,

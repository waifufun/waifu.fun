...
export class SolanaIndexer {
	...
	private extractAmountGotten(
		...
		try {
			...
			const accountKeys =
				transaction.transaction?.message?.staticAccountKeys || transaction.message?.staticAccountKeys || [];
			...
			if (direction === 0) {
				...
				const userIndex = accountKeys.findIndex((key) => key.toBase58() === userAddress);

...
export default function ProfileHeader({
	...
	// };
	return (
		<div className="bg-black/30 border-2 border-[#00FF87]/40 rounded-none shadow-[4px_4px_0px_rgba(0,255,135,0.3)] md:max-h-[182px] md:max-w-full space-y-1 text-white flex flex-col md:flex-row items-center justify-between p-4 w-full mx-auto gap-0">
			...
			<div className="md:ml-6 space-y-1 mb-2 flex flex-col h-full">
				...
				<div className="flex w-full max-w-[320px] max-h-[30px] items-center justify-center sm:justify-start gap-1 mt-1 text-sm font-mono text-gray-400 bg-black/40 py-1 border border-[#00FF87]/30 rounded-none shadow-inner">
					<span className="p-2 truncate">{data.address}</span>
					<CopyButton
						textToCopy={data.address}
						className=" mr-2 text-[#00FF87]/70 hover:text-[#00FF87] cursor-pointer flex-shrink-0"
					/>
				</div>
				{data?.totalPoints ? (
			...
			<div className="h-full flex flex-col md:flex-col w-full space-y-2 place-self-start md:w-fit place-content-center ml-auto">
				<div className="bg-[#00FF87]/10 text-xs text-[#00FF87] border border-[#00FF87]/50 px-2 py-1 rounded-none shadow-[2px_2px_0px_rgba(0,255,135,0.2)] text-center">

...
export default function ProfileHeader({
	...
	// };
	return (
		<div className="bg-black/30 border-2 border-[#E8762D]/40 rounded-none shadow-[4px_4px_0px_rgba(255,45,120,0.3)] md:max-h-[182px] md:max-w-full space-y-1 text-white flex flex-col md:flex-row items-center justify-between p-4 w-full mx-auto gap-0">
			...
			<div className="md:ml-6 space-y-1 mb-2 flex flex-col h-full">
				...
				<div className="flex w-full max-w-[320px] max-h-[30px] items-center justify-center sm:justify-start gap-1 mt-1 text-sm font-mono text-gray-400 bg-black/40 py-1 border border-[#E8762D]/30 rounded-none shadow-inner">
					<span className="p-2 truncate">{data.address}</span>
					<CopyButton
						textToCopy={data.address}
						className=" mr-2 text-[#E8762D]/70 hover:text-[#E8762D] cursor-pointer flex-shrink-0"
					/>
				</div>
				{data?.totalPoints ? (
					<div className="flex flex-col mt-1">
						<div className="flex justify-between items-center w-full px-0 py-1">
							...
							<p className="font-semibold text-[#E8762D] text-base">{formatNumber(data.totalPoints, false, true)}</p>
						</div>
						<div className="flex justify-between items-center w-full px-0 py-1">
							<div className="flex items-center gap-1">
			...
			<div className="h-full flex flex-col md:flex-col w-full space-y-2 place-self-start md:w-fit place-content-center ml-auto">
				<div className="bg-[#E8762D]/10 text-xs text-[#E8762D] border border-[#E8762D]/50 px-2 py-1 rounded-none shadow-[2px_2px_0px_rgba(255,45,120,0.2)] text-center">

...
export default function PointsCounter({ address }: { address: AddressLike }) {
	const query = useQuery({
		...
		queryFn: async () => await getAddressPoints({ address }),
		enabled: !!address,
	});
	...
	return (
		<div className="flex flex-col space-y-1 px-3 text-xs w-full">
			<div className="flex justify-between w-full text-white">
				<span className="font-semibold">{query?.isPending ? "Loading" : formatNumber(weeklyPoints, false, true)}</span>
				<span>WP</span>
			</div>
			<div className="flex justify-between w-full text-white">
				<span className="text-yellow-400 font-semibold">
					{query?.isPending ? "Loading" : formatNumber(points, false, true)}
				</span>
				<span>PP</span>
			</div>
		</div>

...
export const GridItem = ({ token }: { token: IToken }) => {
	return (
		<Link
			...
			className="bg-autofun-background-card group rounded-xl overflow-hidden"
		>
			<div className="flex flex-col min-w-0 relative">
				...
				<div className="absolute left-0 bottom-0 p-2 px-3 w-full z-10">
					<div className="flex items-center gap-4 justify-between">
						<div className="flex items-center gap-2 w-full min-w-0">
							<div className="bg-autofun-background-muted/65 px-1.5 text-autofun-text-primary text-lg font-medium rounded-xl uppercase leading-normal tracking-widest truncate min-w-0 drop-shadow-[0_0px_2px_rgba(0,0,0,0.4)] z-[2]">
								${token.ticker}
							</div>
							<Verified isVerified={token?.verified} />
						</div>
						{token?.createdAt ? (
							<div className="px-1.5 bg-autofun-background-muted/65 rounded-xl text-autofun-text-primary text-sm shrink-0 font-medium  drop-shadow-[0_0px_2px_rgba(0,0,0,0.4)] z-[2]">
				...
				<div className="flex flex-col w-full min-w-0 z-10">
					<div className="absolute flex flex-col top-0 right-0 p-2 px-3 items-end min-w-0 gap-2">
						<div className="bg-autofun-background-muted/65 px-1.5 text-autofun-text-highlight text-base font-medium leading-7 rounded-xl truncate drop-shadow-[0_0px_2px_rgba(0,0,0,0.4)] z-[2]">
							MC {abbreviateNumber(token.marketcap)}
						</div>
						<div className="bg-autofun-background-muted/65 px-1.5 text-autofun-text-primary text-base font-medium leading-7 rounded-xl truncate drop-shadow-[0_0px_2px_rgba(0,0,0,0.4)] z-[2]">

"use client";
import { motion } from "framer-motion";
import { PresaleCard } from "./presale-card";
import type { IPresale } from "@autofun/types";
import { useInfiniteQuery } from "@tanstack/react-query";
import { getPresales } from "@/lib/api";
import { Fragment, useEffect, useRef } from "react";
import { LoaderCircle } from "lucide-react";
import { useSearchParams } from "next/navigation";

export default function PresaleGrid({ presales }: { presales: IPresale[] }) {
	const columns = 5;
	const columnKeys = Array.from({ length: columns }, (_, i) => `col${i + 1}`);

	const searchParams = useSearchParams();
	const category = searchParams.get("category") || null;
	const status = searchParams.get("status") || null;
	const chain = searchParams.get("chain") || null;

	const { data, fetchNextPage, hasNextPage, isFetchingNextPage } = useInfiniteQuery({
		queryKey: ["incubator-presales", category, status, chain],
		queryFn: async ({ pageParam = 1 }) => {
			const res = await getPresales({
				searchParams: {
					page: pageParam,
					category: category ?? undefined,
					status: status ?? undefined,
					chain: chain ?? undefined,
				},
			});
			return res as IPresale[];
		},
		getNextPageParam: (lastPage, allPages) => {
			return lastPage.length < 50 ? undefined : allPages.length + 1;
		},
		initialPageParam: 1,
		initialData: {
			pages: [presales],
			pageParams: [1],
		},
		refetchOnMount: false,
	});

	const allPresales: IPresale[] = data?.pages.flat() ?? [];
	const loaderRef = useRef<HTMLDivElement | null>(null);

	useEffect(() => {
		const observer = new IntersectionObserver(
			(entries) => {
				if (entries[0]?.isIntersecting && hasNextPage && !isFetchingNextPage) {
					fetchNextPage();
				}
			},
			{ threshold: 1.0 },
		);

		if (loaderRef.current) observer.observe(loaderRef.current);

		return () => {
			if (loaderRef.current) observer.unobserve(loaderRef.current);
		};
	}, [hasNextPage, isFetchingNextPage, fetchNextPage]);

	return (
		<Fragment>
			<div className="columns-2 sm:columns-2 lg:columns-3 xl:columns-4 2xl:columns-5 gap-4 space-y-4 w-full">
				{columnKeys.map((colKey, colIndex) => {
					const columnItems = allPresales?.filter((_, idx) => idx % columns === colIndex);
					return (
						<motion.div
							key={colKey}
							className="flex flex-col space-y-4"
							initial={{ opacity: 0, y: -250, rotateX: 50 }}
							animate={{ opacity: 1, y: 0, rotateX: 0 }}
							transition={{
								delay: colIndex * 0.35,
								type: "spring",
								stiffness: 25,
								damping: 7,
							}}
						>
							{columnItems?.map((presale) => (
								<PresaleCard key={presale._id?.toString()} presale={presale} />
							))}
						</motion.div>
					);
				})}
				<div ref={loaderRef} className="h-10 w-full" />
			</div>
			<div>
				{isFetchingNextPage && (
					<div className="mt-2 place-items-center">
						<div className="w-full flex justify-center animate-spin text-muted-foreground">
							<LoaderCircle className="h-8 w-8 text-[#03FF23]" />
						</div>
					</div>
				)}
			</div>
		</Fragment>
	);
}

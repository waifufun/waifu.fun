...
export function FormSection({ title, children, className, collapsible = false, defaultOpen = true }: FormSectionProps) {
	return (
		<div
			className={cn(
				"bg-black/30 border-2 border-[#FF2D78]/40 rounded-none shadow-[4px_4px_0px_rgba(255,45,120,0.3)]",
			...
			{collapsible && title ? (
				<Accordion type="single" collapsible defaultValue={defaultOpen ? "item-1" : ""} className="w-full">
					<AccordionItem value="item-1" className="border-b-0">
						<AccordionTrigger
							className={cn(
								"p-3 md:p-4 hover:no-underline focus-visible:ring-1 focus-visible:ring-[#FF2D78] focus-visible:ring-offset-2 focus-visible:ring-offset-black rounded-none data-[state=open]:bg-[#FF2D78]/10 text-sm font-bold text-[#FF2D78] uppercase tracking-wider cursor-pointer",
								"[&>svg]:text-[#FF2D78]/70 [&>svg]:hover:text-[#FF2D78]",
							)}
						>
				...
				<>
					{title && (
						<div className="flex justify-between items-center mb-3">
							<h2 className="text-sm font-bold text-[#FF2D78] uppercase tracking-wider">{title}</h2>
						</div>
					)}
					<div className="space-y-3">{children}</div>
				</>

...
import { ChevronDown, Edit3, User, Activity, Loader2, RefreshCw, Wallet } from "lucide-react";
	...
	return (
		<div className="relative bg-[#111114] border border-[rgba(255,255,255,0.06)] rounded-sm overflow-hidden">
				...
				<AnimatePresence mode="wait">
					{isGeneratingImage ? (
						...
						>
							...
							<span className="text-[10px] font-mono text-[#00ff87] uppercase tracking-widest">generating</span>
						</motion.div>
					) : imageUrl ? (
						<motion.img
							key={imageUrl}
			...
			<div className="p-4 space-y-3">
				{/* Name + Ticker */}
				<div>
					<motion.h3 layout className="text-[#e4e4e7] font-bold text-lg truncate">
				...
				{character.description && (
					<p className="text-xs text-[#71717a] leading-relaxed line-clamp-2">{character.description}</p>
				)}
				...
				<div className="bg-[#08080a] border border-[rgba(255,255,255,0.04)] rounded-sm p-3">
					<div className="flex items-start gap-2">
						...
						<div className="flex-1 min-w-0">
							<p className="text-[10px] text-[#52525b] mb-0.5">{character.name || "Agent"} &middot; just now</p>
							<p className="text-xs text-[#71717a] leading-relaxed">{samplePost}</p>
						</div>
					</div>
				</div>
	...
	return (
		<div className="bg-[#111114] border border-[rgba(255,255,255,0.06)] rounded-sm overflow-hidden">
			...
			>
				...
				<motion.div animate={{ rotate: isOpen ? 180 : 0 }} transition={{ type: "spring", stiffness: 200, damping: 25 }}>
					...
					>
						<div className="px-4 pb-4 space-y-4 border-t border-[rgba(255,255,255,0.04)]">
							...
							<div className="pt-3">
								...
								{errors.buyAmount && <p className="text-red-500 text-[10px] mt-1">{errors.buyAmount.message}</p>}
								<div className="flex justify-between items-center mt-1">
									<div className="flex items-center gap-1">
										<Wallet className="w-3 h-3 text-[#52525b]" />
										<span className="text-[10px] text-[#52525b]">{balance.toFixed(4)} BNB</span>
									</div>
									<button
										type="button"
				...
				<div>
					...
					<div className="relative">
						<Input
							...
							maxLength={5}
						/>
						<span className="absolute left-2 top-1/2 -translate-y-1/2 text-[#00ff87] font-bold text-xs mt-0.5">$</span>
					</div>
				</div>
			</div>
			<div>
				...
				<textarea
					...
					className={cn(formElementBaseClass, "mt-1 w-full px-3 py-2 text-xs resize-none min-h-[60px]")}
					maxLength={200}
				/>
			</div>
			<button
...
export function CharacterPreviewPanel({
	...
	return (
		...
		>
			{/* Agent card */}
			<AgentCardPreview character={character} imageUrl={imageUrl} isGeneratingImage={isGeneratingImage} />
			...
			{/* Inline editor */}
			<InlineFieldEditor character={character} onUpdate={onCharacterUpdate} />

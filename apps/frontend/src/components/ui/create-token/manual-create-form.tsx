...
const UploadPlaceholder = ({
	...
	return (
		<button
			type="button"
			onClick={onClick}
			className={`w-full h-full border-2 border-dashed rounded-none shadow-[3px_3px_0px_rgba(0,255,135,0.3)] flex flex-col items-center justify-center p-4 text-center cursor-pointer transition-all group focus:outline-none focus:ring-2 focus:ring-[#00FF87] ${
				isDragActive
		...
		>
			<UploadCloud
				...
				className={`mb-2 transition-all ${
					isDragActive ? "text-[#00FF87] scale-110" : "text-[#00FF87]/70 group-hover:text-[#00FF87]"
				}`}
			/>
			<p
	...
	return (
		<FormSection title="Token Image" className="space-y-4" collapsible={false}>
					...
					<div className="flex gap-2">
						{!uploadedImage && previousImages.length > 0 && (
							<button
								...
								disabled={isConverting}
								className="text-xs text-[#00FF87] hover:text-white disabled:text-gray-500 disabled:cursor-not-allowed transition-colors hover:underline"
							>
			...
			{!displayImage && previousImages.length > 0 && (
				<div className="flex items-center justify-between p-2 bg-[#00FF87]/10 border border-[#00FF87]/30 rounded-none">
					<p className="text-xs text-gray-300">Generated image available from Auto tab</p>
					<button
						type="button"
						onClick={handleUseGeneratedImage}
						disabled={isConverting}
						className="text-xs text-[#00FF87] hover:text-white disabled:text-gray-500 disabled:cursor-not-allowed transition-colors hover:underline font-semibold"
					>

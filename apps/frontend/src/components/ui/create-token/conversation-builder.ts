...
export type ConversationPhase = "greeting" | "concept" | "personality" | "appearance" | "refine" | "ready";
...
function detectIntent(text: string, phase: ConversationPhase): Intent {
	...
	// Acceptance signals
	if (
		/^(yes|yep|yeah|looks good|perfect|love it|ship it|let'?s go|ok|okay|sure|accepted?|lgtm|nice|great|good)\s*[.!]?\s*$/i.test(
			lower,
		)
	) {
		return { type: "accept" };
	...
	const nameMatch = lower.match(
		/(?:name\s*(?:it|her|him|them)?\s*[:=]?\s*|call\s*(?:it|her|him|them)\s+)["']?([a-z0-9 ]+)["']?/i,
	);
	if (nameMatch) {
...
function generateConceptResponse(
	text: string,
	state: BuilderState,
): {
	content: string;
	...
	const adjectiveStr = adjectives.length > 0 ? adjectives.slice(0, 3).join(", ") : "interesting";
...
export class ConversationBuilder {
	...
	processMessage(userText: string): BuilderMessage {
		...
		switch (intent.type) {
			...
			case "request_image": {
				const prompt =
					this.state.character.avatarPrompt || this.state.character.personality || this.state.character.description;
				if (!prompt) {
					return {
						id: uid(),
						role: "assistant",
			...
			default: {
				...
				if (this.state.character.description) {
					...
					const refined = currentDesc.length + userText.length < 200 ? `${currentDesc}. ${userText}` : userText;
					const delta: Partial<AgentCharacter> = {

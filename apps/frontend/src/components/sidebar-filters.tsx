	...
	return (
		<>
			<Collapsible open={showFiltersPanel && !isCollapsed} onOpenChange={setShowFiltersPanel}>
				...
				>
					<div className="border-t border-[rgba(255,255,255,0.06)] mt-2 transition-opacity duration-200" />
						...
						<div className="w-full">
							...
							<Select open={sourceOpen} onOpenChange={setSourceOpen}>
								<SelectTrigger
									...
									className="w-full bg-[#08080a] border border-[rgba(255,255,255,0.06)] rounded-sm  uppercase text-[11px] mt-1 h-8 transition-all duration-200 hover:border-[#00ff87]/30"
								>
									<SelectValue placeholder={searchParams.get("origin") || "Select Source"} />
								</SelectTrigger>
								<SelectContent className="bg-[#08080a] border border-[rgba(255,255,255,0.06)] text-gray-50 rounded-sm  uppercase text-[11px] animate-in fade-in-0 zoom-in-95">
									{sourceOptions.map((option) => (
										<Link
											className={cn(
												"hover:bg-[rgba(0,255,135,0.08)] focus:bg-accent focus:text-accent-foreground",

	...
	return (
		<>
			<Collapsible open={showFiltersPanel && !isCollapsed} onOpenChange={setShowFiltersPanel}>
				<CollapsibleTrigger
					className={cn(
						"text-white hover:bg-[#00FF87]/10 hover:text-[#00FF87] py-4.5 cursor-pointer transition-all duration-200",
				...
				>
					<div className="border-t border-[#00FF87]/20 mt-2 transition-opacity duration-200" />
					<div className="p-2 space-y-4 mt-4 text-[11px] uppercase font-semibold text-gray-400">
						<div className="w-full">
							...
							<Select open={sourceOpen} onOpenChange={setSourceOpen}>
								<SelectTrigger
									...
									className="w-full bg-black border-2 border-[#00FF87]/80 rounded-none shadow-[3px_3px_0px_rgba(0,255,135,0.45)] uppercase text-[11px] mt-1 h-8 transition-all duration-200 hover:border-[#00FF87] hover:shadow-[4px_4px_0px_rgba(0,255,135,0.6)]"
								>
									<SelectValue placeholder={searchParams.get("origin") || "Select Source"} />
								</SelectTrigger>
								<SelectContent className="bg-black border-2 border-[#00FF87]/80 text-gray-50 rounded-none shadow-[3px_3px_0px_rgba(0,255,135,0.45)] uppercase text-[11px] animate-in fade-in-0 zoom-in-95">
									{sourceOptions.map((option) => (
										<Link
											className={cn(
												"hover:bg-[#00FF87]/30 focus:bg-accent focus:text-accent-foreground",
						...
						<div>
							...
							<Select open={statusOpen} onOpenChange={setStatusOpen}>
								...
								<SelectContent className="bg-black border-2 border-[#00FF87]/80 text-gray-50 rounded-none shadow-[4px_4px_0px_rgba(0,255,135,0.45)] uppercase text-[11px] animate-in fade-in-0 zoom-in-95">

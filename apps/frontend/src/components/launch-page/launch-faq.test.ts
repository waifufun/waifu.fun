import { describe, expect, it } from "vitest";

import { LAUNCH_TIERS } from "@/lib/launch-vault/tiers";
import enMessages from "@/locales/en.json";

import { type FaqItem, buildLaunchFaq } from "./launch-faq-data";

type Messages = Record<string, unknown>;
const en = enMessages as Messages;

function getNested(obj: Messages, path: string): string {
	const keys = path.split(".");
	let current: unknown = obj;
	for (const key of keys) {
		if (current == null || typeof current !== "object") return "";
		current = (current as Record<string, unknown>)[key];
	}
	return typeof current === "string" ? current : "";
}

function interpolate(template: string, params?: Record<string, string>): string {
	if (!params) return template;
	let out = template;
	for (const [k, v] of Object.entries(params)) {
		out = out.replace(new RegExp(`\\{\\{${k}\\}\\}`, "g"), v);
	}
	return out;
}

function resolve(item: FaqItem): { q: string; a: string } {
	return {
		q: getNested(en, item.qKey),
		a: interpolate(getNested(en, item.aKey), item.params),
	};
}

describe("buildLaunchFaq", () => {
	it("returns 4 questions regardless of tier", () => {
		for (const tier of Object.values(LAUNCH_TIERS)) {
			const items = buildLaunchFaq(tier);
			expect(items).toHaveLength(4);
		}
	});

	it("every item resolves to a non-empty english string", () => {
		for (const tier of Object.values(LAUNCH_TIERS)) {
			for (const item of buildLaunchFaq(tier)) {
				const { q, a } = resolve(item);
				expect(q.length, `q for ${item.qKey}`).toBeGreaterThan(0);
				expect(a.length, `a for ${item.aKey}`).toBeGreaterThan(0);
				// interpolated params should not leak placeholders
				expect(a).not.toMatch(/\{\{[^}]+\}\}/);
			}
		}
	});

	it("tier 80 does not pitch v2 lp graduation", () => {
		const items = buildLaunchFaq(LAUNCH_TIERS.TIER_80);
		const body = items.map((i) => resolve(i).a).join(" ");
		// negative copy is fine ("no separate v2 lp"); we just don't pitch graduation.
		expect(body).not.toMatch(/added to pcs v2 lp/i);
		expect(body).not.toMatch(/graduates to pcs v2/i);
		expect(body).toMatch(/bonding curve/i);
	});

	it("tier 80 claim copy is 100% at tge (no vesting)", () => {
		const items = buildLaunchFaq(LAUNCH_TIERS.TIER_80);
		const claim = items.map(resolve).find((i) => i.q === "when can i claim?")?.a ?? "";
		expect(claim).toMatch(/100% unlocks at tge/);
		expect(claim).not.toMatch(/24h/);
	});

	it("tier 90 mentions v2 lp + 24h vesting", () => {
		const items = buildLaunchFaq(LAUNCH_TIERS.TIER_90);
		const resolved = items.map(resolve);
		const bnb = resolved.find((i) => i.q === "what happens to the bnb?")?.a ?? "";
		const claim = resolved.find((i) => i.q === "when can i claim?")?.a ?? "";
		expect(bnb).toMatch(/pcs v2 lp/);
		expect(bnb).toMatch(/90%/);
		expect(claim).toMatch(/24h/);
		expect(claim).toMatch(/50% unlocks/);
	});

	it("tier 98 surfaces its display name in burn copy", () => {
		const items = buildLaunchFaq(LAUNCH_TIERS.TIER_98);
		const burn = items.map(resolve).find((i) => i.q === "what gets burned?")?.a ?? "";
		expect(burn).toMatch(/GIGACHAD/);
		expect(burn).toMatch(/320m/);
	});

	it("refund question is uniform across tiers", () => {
		for (const tier of Object.values(LAUNCH_TIERS)) {
			const items = buildLaunchFaq(tier);
			const refund = items.map(resolve).find((i) => i.q === "what if the bundle fails?")?.a ?? "";
			expect(refund).toMatch(/refunds enable/i);
			expect(refund).toMatch(/bonus pool/i);
		}
	});

	it("contains no em-dashes in either keys or resolved text", () => {
		for (const tier of Object.values(LAUNCH_TIERS)) {
			for (const item of buildLaunchFaq(tier)) {
				const { q, a } = resolve(item);
				expect(q.includes("\u2014"), `q has em-dash: ${q}`).toBe(false);
				expect(a.includes("\u2014"), `a has em-dash: ${a}`).toBe(false);
			}
		}
	});
});

...
function StatusPill({ label, tone }: { label: string; tone?: string | undefined }) {
	return (
		<span className={cn("inline-flex items-center gap-1 rounded-sm border px-2 py-0.5 text-[10px] font-mono uppercase tracking-[0.18em]", tone ?? TONE.none)}>
			{label}
		</span>
	);
}
...
export default function OwnerRuntimePanel({ token }: { token: IToken }) {
	...
	const [billingMode, setBillingMode] = useState<NonNullable<OwnerTokenRuntime["billingMode"]>>(token.billingMode ?? "owner_credits");
	...
	useEffect(() => {
		...
		const t = window.setTimeout(() => { void authQuery.refetch(); }, 1_500);
	...
	const rtQuery = useQuery({
		queryKey: rtQK,
		queryFn: () => getOwnerTokenRuntime({ chain: token.chain, chainId: token.chainId, contractAddress: token.contractAddress }),
		enabled: canManage,
		retry: false,
	});
	...
	const claimMut = useMutation({
		mutationFn: () => claimTokenOwnership({ chain: token.chain, chainId: token.chainId, contractAddress: token.contractAddress }),
		onSuccess: async () => { toast.success("Owner access claimed"); await refreshAll(); },
		onError: (e: Error) => toast.error(fmtErr(e)),
	});

	const activateMut = useMutation({
		mutationFn: () => activateOwnerTokenRuntime({
			chain: token.chain, chainId: token.chainId, contractAddress: token.contractAddress,
			billingMode,
			character: { name: token.name, ...(token.description ? { bio: token.description } : {}), ...(token.image ? { avatar: token.image } : {}) },
		}),
		onSuccess: async () => { toast.success("Runtime activation requested"); await refreshAll(); },
		onError: (e: Error) => toast.error(fmtErr(e)),
	});

	const suspendMut = useMutation({
		mutationFn: () => suspendOwnerTokenRuntime({ chain: token.chain, chainId: token.chainId, contractAddress: token.contractAddress }),
		onSuccess: async () => { toast.success("Runtime suspended"); await refreshAll(); },
		onError: (e: Error) => toast.error(fmtErr(e)),
	});

	const resumeMut = useMutation({
		mutationFn: () => resumeOwnerTokenRuntime({ chain: token.chain, chainId: token.chainId, contractAddress: token.contractAddress }),
		onSuccess: async () => { toast.success("Runtime resumed"); await refreshAll(); },
		onError: (e: Error) => toast.error(fmtErr(e)),
	});
	...
	const reserveUsd = runtime?.infraReserveUsd ?? token.infraReserveUsd;
		...
		<div className="relative overflow-hidden rounded-sm border border-[rgba(255,255,255,0.06)] bg-[#111114] p-3 sm:p-4 transition-colors hover:border-[rgba(255,255,255,0.12)]">
			<div className="flex flex-col gap-3">
				...
				<div className="flex items-center justify-between gap-2">
					...
					<Button variant="outline" size="sm" onClick={() => { void refreshAll(); }} disabled={anyPending}
				...
				{!isClaimed && (
					<div className="flex items-center gap-2 flex-wrap rounded-sm border border-white/6 bg-[#0a0a0d] p-2.5">
						...
						{canClaim ? (
							<Button onClick={() => claimMut.mutate()} disabled={claimMut.isPending || !creatorAddress}
								className="ml-auto h-7 px-3 text-[10px] font-mono uppercase">
								{claimMut.isPending ? <LoaderCircle className="size-3 animate-spin" /> : <CheckCircle2 className="size-3" />}
								claim owner access
							</Button>
						) : (
							<span className="ml-auto text-[10px] text-[#52525b] font-mono">
								{authQuery.isLoading ? "checking auth…" : isConnectedCreator ? "finish wallet auth to claim" : "connect creator wallet"}
							</span>
						)}
					</div>
				)}
						...
						<div className="flex items-center gap-2 flex-wrap">
							...
							{typeof reserveUsd === "number" && !Number.isNaN(reserveUsd) && (
								<span className="text-[10px] font-mono text-[#52525b]">
									reserve: {formatNumber(reserveUsd, true)}
								</span>
							)}
						</div>
							...
							<div className="flex items-center gap-2 flex-wrap">
								{(runtimeStatus === "none" || runtimeStatus === "failed" || runtimeStatus === "deleted") && (
									<Button onClick={() => activateMut.mutate()} disabled={activateMut.isPending}
										...
										{activateMut.isPending ? <LoaderCircle className="size-3 animate-spin" /> : <PlayCircle className="size-3" />}
										activate
									</Button>
								)}
								{runtimeStatus === "running" && (
									<Button variant="outline" onClick={() => suspendMut.mutate()} disabled={suspendMut.isPending}
										className="h-7 px-3 text-[10px] font-mono uppercase text-amber-300 hover:text-amber-200">
										{suspendMut.isPending ? <LoaderCircle className="size-3 animate-spin" /> : <PauseCircle className="size-3" />}
										suspend
									</Button>
								)}
								{runtimeStatus === "suspended" && (
									<Button onClick={() => resumeMut.mutate()} disabled={resumeMut.isPending}
										className="h-7 px-3 text-[10px] font-mono uppercase">
										{resumeMut.isPending ? <LoaderCircle className="size-3 animate-spin" /> : <RefreshCw className="size-3" />}
										resume
									</Button>
								)}
								{runtimeStatus === "provisioning" && (
									<span className="flex items-center gap-1.5 text-[10px] font-mono text-sky-300">
										<LoaderCircle className="size-3 animate-spin" />provisioning…
									</span>
								)}

								{/* billing — single line, not a full section */}

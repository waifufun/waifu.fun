...
export default function Chat({ token }: { token: IToken }) {
	...
	return (
		<div className="bg-black border-2 border-[#00FF87]/40 rounded-none shadow-[4px_4px_0px_rgba(0,255,135,0.3)] flex flex-col h-[550px]">
			<div className="flex items-center justify-between p-2 border-b-2 border-[#00FF87]/30 bg-black/50">
					...
					<TabsList shadowed={false} className="bg-transparent border-0 p-0 h-auto">
						{["1000", "100000", "1000000"].map((r) => (
							<TabsTrigger
								...
								filled={false}
								className={cn(
									"text-xs font-bold uppercase tracking-wider rounded-none px-3 py-1.5 h-auto relative data-[state=active]:text-[#00FF87] data-[state=inactive]:text-gray-500 hover:text-gray-300 bg-transparent border-0",
								)}
							>
...
const ChatWindow = ({ token, room, tierRequirement }: { room: TChatRooms; token: IToken; tierRequirement: string }) => {
	...
	return (
		<>
			...
			<div className="p-2 border-t-2 border-[#00FF87]/30 bg-black/50">
				{imagePreviewUrl && (
					<div className="relative w-fit mb-2">
						<button
							type="button"
							className="absolute bg-black/80 top-1 right-1 cursor-pointer size-6 p-1 inline-flex rounded-none border border-[#00FF87]/30"
							onClick={() => {
						...
						<Image
							className="max-w-[200px] max-h-[150px] object-cover border-2 border-[#00FF87]/30 rounded-none shadow-[2px_2px_0px_rgba(0,255,135,0.2)]"
							src={imagePreviewUrl}
				...
				<form className="flex items-center gap-2" onSubmit={handleSubmit(onSubmit)}>
					...
					<Button
						...
						className="h-9 w-9 p-0 text-gray-400 hover:text-[#00FF87] rounded-none flex-shrink-0"
					>
						<ImageIcon size={18} />
					</Button>
					...
					<Input
						...
						className="bg-black border-2 border-[#00FF87]/50 placeholder-gray-600 text-sm h-9 focus:border-[#00FF87] text-gray-200 rounded-none shadow-[2px_2px_0px_rgba(0,255,135,0.2)] flex-grow"
					...
					<Button
						...
						className="h-9 w-9 p-0 bg-[#00FF87]/80 hover:bg-[#00FF87] text-black rounded-none shadow-[2px_2px_0px_#01a718] flex-shrink-0"
					>
						<Send size={18} />
					</Button>
				</form>
...
const ChatItem = ({ message }: { message: IChatMessage }) => {
	return (
		...
		>
			<Avatar className="h-8 w-8 border-2 border-[#00FF87]/30 rounded-none shadow-[1px_1px_0px_rgba(0,255,135,0.2)]">
				<AvatarImage src="/chain-icons/solana.svg" alt={message?.sender ? shortenAddress(message?.sender) : "User"} />
				<AvatarFallback className="bg-gray-700 text-xs text-[#00FF87] rounded-none">
			...
			<div className="flex-1">
				...
				{message?.image && (
					<div className="mt-2 border-2 border-[#00FF87]/30 rounded-none shadow-[2px_2px_0px_rgba(0,255,135,0.2)] overflow-hidden max-w-xs">

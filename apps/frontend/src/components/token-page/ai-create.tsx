...
const AiCreatePanel = ({ token }: { token: IToken }) => {
	...
	const getMinBalance = (mediaType: string, mode: "standard" | "fast") => {
		if (mediaType === "image") {
			return mode === "fast"
	...
	const handleFullscreen = (mediaUrl: string, type: string) => {
		if (type === "video") {
			...
			const closeFullscreen = () => {
				...
				videoElement.removeEventListener("click", closeFullscreen);
			};
			...
			const handleFullscreenChange = () => {
				if (!document.fullscreenElement && document.body.contains(videoElement)) {
					closeFullscreen();
				}
			};
	...
	const renderModeSelector = () => {
		...
		const standardBalance = getMinBalance(activeAiTab, "standard");
		const fastBalance = getMinBalance(activeAiTab, "fast");
					...
					<Button
						...
						className={`flex-1 text-sm px-3 py-2 h-auto rounded-none border-2 uppercase ${
							generationMode === "standard"
								? "border-[#E8762D] bg-[#E8762D] text-black hover:bg-[#e6266d] hover:text-black"
	...
	const renderTabContent = () => {
		...
		const hasStandardTokens = userTokenAmount >= getMinBalance(activeAiTab, "standard");
					...
					<div className="flex items-center gap-2 p-4 border border-red-500/40 rounded-none bg-red-900/20 text-sm text-red-300 shadow-[2px_2px_0px_rgba(239,68,68,0.2)]">
						...
						<div>
							...
							<p className="text-xs mt-1">
								...
								<span className="text-[#E8762D] font-bold">{token.ticker}</span> tokens to generate {activeAiTab}s.
							</p>
						</div>
					</div>
				</div>
			...
			return (
				<div className="space-y-4">
					...
					<Input
						...
						className="bg-black border-2 border-[#E8762D]/60 placeholder-gray-500 text-sm h-10 focus:border-[#E8762D] focus:ring-1 focus:ring-[#E8762D] text-gray-200 rounded-none shadow-[3px_3px_0px_rgba(255,45,120,0.25)] uppercase tracking-wider"
						{...registerForm("prompt")}
					/>
					<Button
						onClick={() => handleGenerateMedia("image")}
						disabled={isGeneratingMedia || !hasEnoughTokens}
						className="w-full bg-[#E8762D] hover:bg-[#e6266d] text-black hover:text-black font-bold text-sm h-10 rounded-none shadow-[4px_4px_0px_#b31f57] hover:shadow-[2px_2px_0px_#b31f57] active:shadow-none hover:-translate-x-0.5 hover:-translate-y-0.5 active:translate-x-0 active:translate-y-0 uppercase tracking-wider disabled:opacity-50 disabled:cursor-not-allowed"
					>
						{isGeneratingMedia ? (
							<>
					...
					<div>
						...
						<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
							{previousImages.slice(0, 12).map((img, index) => (
								<div
									key={`image-${img}`}
									className="relative group border-2 border-[#E8762D]/40 rounded-none shadow-[3px_3px_0px_rgba(255,45,120,0.25)] overflow-hidden"
								>
									<Image
									...
									/>
									<div className="absolute bottom-0 left-0 right-0 bg-black/70 p-1 flex justify-end gap-1 transition-opacity">
										<Button
											variant="ghost"
											size="icon"
											onClick={() => handleFullscreen(img, "image")}
											className="h-6 w-6 p-1 text-gray-300 hover:text-[#E8762D] rounded-none"
					...
					<div>
						...
						<div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
							{previousVideos.slice(0, 12).map((video, index) => (
								<div
									key={`video-${video}`}
									className="relative group border-2 border-[#E8762D]/40 rounded-none shadow-[3px_3px_0px_rgba(255,45,120,0.25)] overflow-hidden aspect-[4/3]"
								>
									<video
										src={video}
	...
	return (
		<div className="space-y-4 mx-4 mt-4">
			<div className="flex gap-1 border-b-2 border-[#E8762D]/30 pb-2">
				{["image", "video"].map((tab) => (
					<Button
						key={tab}
						...
						className={`capitalize text-sm px-3 py-1.5 h-auto rounded-none border-2 ${
							activeAiTab === tab
								? "border-black bg-[#E8762D] text-black hover:bg-[#e6266d] hover:text-black"
								: "border-transparent text-gray-300 hover:text-gray-300 hover:bg-[#E8762D]/10 hover:border-[#E8762D]/50"
						}`}
					>
...
export default function TokenCreatePageClient({ token }: { token: IToken }) {
	return (
		<PromptProvider tokenImageQuery={`${token.contractAddress}-${token.chainId}-images`}>
			<TokenCreatePageContent token={token} />
		</PromptProvider>
	);
}

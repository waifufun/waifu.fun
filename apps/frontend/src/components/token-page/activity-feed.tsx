...
function TradeCard({ trade, index }: { trade: MockTrade; index: number }) {
	...
	return (
		<motion.div
			...
			className={cn(
				"relative flex-shrink-0 inline-flex items-center gap-2 px-3 py-2 rounded-sm text-xs font-mono whitespace-nowrap cursor-default",
				"bg-[#111114] border transition-all duration-200",
				isBuy
					? "border-[#22c55e]/20 hover:border-[#22c55e]/40 hover:shadow-[0_0_12px_rgba(34,197,94,0.1)]"
					: "border-red-500/20 hover:border-red-500/40 hover:shadow-[0_0_12px_rgba(239,68,68,0.1)]",
			)}
		>
		...
		return (
			<div className="w-full">
				<div className="flex items-center gap-2 mb-2">
					<Zap className="size-3.5 text-[#52525b]" />
					<span className="text-[10px] text-[#52525b] font-mono uppercase tracking-wider">recent activity</span>
				</div>
				<div className="flex items-center justify-center py-6 px-4 bg-[#111114] border border-[rgba(255,255,255,0.06)] rounded-sm">
		...
		<div className="w-full overflow-hidden">
			<div className="flex items-center gap-2 mb-2">
				<div className="relative flex items-center justify-center">
					<span className="absolute h-3 w-3 rounded-full bg-[#00ff87]/30 animate-ping" />
					<span className="relative h-1.5 w-1.5 rounded-full bg-[#00ff87] shadow-[0_0_6px_rgba(0,255,135,0.6)]" />
				</div>
				<span className="text-[10px] text-[#52525b] font-mono uppercase tracking-wider">recent activity</span>
			...
			<div className="relative">
				...
				<div className="flex gap-2 overflow-x-auto pb-1 px-2 scrollbar-hide">
					{trades.map((trade, i) => (
						<TradeCard key={`${trade.type}-${i}`} trade={trade} index={i} />
					))}
				</div>
			</div>
		</div>

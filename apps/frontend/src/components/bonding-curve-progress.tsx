...
function AnimatedProgressBar({ value, max }: { value: number; max: number }) {
	...
	useEffect(() => { const timer = setTimeout(() => setWidth(percentage), 100); return () => clearTimeout(timer); }, [percentage]);

	return (
		<div className="relative">
			<div className="absolute inset-0 flex items-center pointer-events-none z-10">
				{MILESTONES.map((milestone) => (
					<div key={milestone.percent} className="absolute flex flex-col items-center" style={{ left: `${milestone.percent}%`, transform: "translateX(-50%)" }}>
						<div className={cn("w-0.5 h-5 -mt-0.5 rounded-full transition-colors duration-500", percentage >= milestone.percent ? "bg-[#00ff87]" : "bg-[rgba(255,255,255,0.1)]")} />
					</div>
				))}
			</div>
			<div className="relative h-5 w-full bg-[rgba(0,255,135,0.06)] rounded-sm overflow-hidden border border-[rgba(255,255,255,0.06)]">
				<motion.div className="absolute inset-y-0 left-0 bg-gradient-to-r from-[#065f46] via-[#22c55e] to-[#00ff87] rounded-sm" initial={{ width: 0 }} animate={{ width: `${width}%` }} transition={{ duration: 1, ease: "easeOut" }}>
					<div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent animate-shimmer" />
				</motion.div>
				{width > 0 && (
					<motion.div className="absolute top-0 bottom-0 w-3" style={{ left: `calc(${width}% - 6px)` }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }}>
				...
				<div className="absolute inset-0 flex">{[...Array(10)].map((_, i) => (<div key={i} className="flex-1 border-r border-[rgba(255,255,255,0.03)] last:border-r-0" />))}</div>
			</div>
					...
					return (
						<Tooltip key={milestone.percent}>
							<TooltipTrigger asChild>
								<div className={cn("flex items-center gap-1 cursor-default transition-all duration-300", reached ? "text-[#00ff87]" : "text-[#3f3f46]")}>
							...
							<TooltipContent><span className="text-xs">{milestone.percent}% — {milestone.label}</span></TooltipContent>
						</Tooltip>
					);
				})}
			</div>
...
export default function BondingCurveProgress({ token, title, showTooltip }: { token: IToken; title?: string; showTooltip?: boolean }) {
			...
			<div className="flex flex-col gap-3">
				<div className="flex items-center gap-4 justify-between">
					<div className="flex items-center gap-2">
						...
						<span className="text-[10px] text-[#52525b] font-mono uppercase tracking-wider">{title ? title : "bonding progress"}</span>
					</div>
					<div className="flex items-center gap-2">
						<motion.span className="text-sm font-bold text-[#00ff87] font-mono" key={curveProgress} initial={{ scale: 1.2, opacity: 0.5 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.3 }}>{curveProgress.toFixed(2)}%</motion.span>
						{showTooltip && (
							<Tooltip>
								<TooltipTrigger><AlertCircle className="text-[#52525b] hover:text-[#a1a1aa] transition-colors" size={14} /></TooltipTrigger>
								<TooltipContent><span className="text-xs">when the market cap reaches the graduation threshold,<br />the coin&apos;s liquidity will transition to raydium.</span></TooltipContent>
							</Tooltip>
						)}
					</div>
				</div>
				...
				{solRequiredForMigration > 0 && (
					<motion.div className="flex flex-wrap items-center gap-x-1 text-xs text-[#a1a1aa] font-mono" initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
						<span className="text-[#52525b]">reserve:</span>
						<span className="text-[#00ff87]">{formatNumber(Number(currentReserveLamports / LAMPORTS_PER_SOL), false, true)} SOL</span>

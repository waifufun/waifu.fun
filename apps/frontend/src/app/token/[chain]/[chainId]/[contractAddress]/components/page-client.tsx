...
export default function PageClient({
	...
	const getBadgeInfo = () => {
		...
		if (initialData?.status === "migrated" || initialData?.status === "locked") {
			return {
				badge: "BONDED",
				classes:
					"bg-black/80 hover:bg-primary/15 text-[#E8762D] border border-[#E8762D]/50 shadow-[1.5px_1.5px 0px_rgba(255,45,120,0.3)] sm:shadow-[2px_2px_0px_rgba(255,45,120,0.3)] py-0.5 px-1.5 text-[9px] sm:text-[10px]",
			};
		}
	...
	return (
		<div className="flex flex-col gap-6 mt-3 container">
			...
			<div className="bg-black border-2 border-[#E8762D]/40 p-3 rounded-none shadow-[4px_4px_0px_rgba(255,45,120,0.3)] flex flex-col lg:flex-row gap-6 items-start lg:items-center justify-between">
				<div className="flex items-center gap-4">
					...
					<Image
						...
						unoptimized
						alt="token_image"
						className="h-10 w-10 rounded-none border-2 border-[#E8762D]/50 shadow-[2px_2px_0px_rgba(255,45,120,0.3)] pixelated-image-render"
					/>
					{/* Token Name */}
					<div className="space-y-3 xl:space-y-0">
						...
						<div className="flex items-center gap-3 flex-wrap">
							...
							<span className="text-lg text-[#E8762D]/80 font-mono animate-subtle-flicker">{token.ticker}</span>
			...
			<div className="flex flex-col lg:flex-row lg:flex-nowrap gap-6">
				<div className="w-full lg:w-7/10 flex flex-col gap-6 order-3 lg:order-2">
					<div className="w-full relative">
						<div className="bg-black border-2 border-[#E8762D]/40 p-1 rounded-none shadow-[4px_4px_0px_rgba(255,45,120,0.3)]">
				...
				<div className="w-full lg:w-3/10 flex flex-col md:flex-row lg:flex-col gap-6 order-2 lg:order-3">
					<Swap token={token} />
					<div className="flex flex-col gap-4 bg-black border-2 border-[#E8762D]/40 p-3 rounded-none shadow-[4px_4px_0px_rgba(255,45,120,0.3)]">
						<BondingCurveProgress token={token} />
						<div className="flex flex-row gap-4 items-start">
							<Image
								src={token?.image}
								className="border-2 max-w-[50px] border-[#E8762D]/50 rounded-none shadow-[2px_2px_0px_rgba(255,45,120,0.25)] pixelated-image-render mt-1 flex-shrink-0"
								unoptimized
								priority
								width={208}
								height={208}
								...
								return (
									...
									<Comp {...compProps} key={social.title}>
										<Image
											src={social.icon}
											className={cn([
												"inline-flex items-center justify-center gap-2 whitespace-nowrap text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0 bg-background h-7 w-7 p-1 border-2 border-[#E8762D]/50 text-[#E8762D]/80 hover:text-[#E8762D] hover:bg-[#E8762D]/10 hover:border-[#E8762D] rounded-none shadow-[2px_2px_0px_rgba(255,45,120,0.2)]",

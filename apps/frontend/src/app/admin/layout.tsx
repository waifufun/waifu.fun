...
export default function AdminLayout({ children }: AdminLayoutProps) {
	...
	if (isLoading) {
		return (
			<div className="min-h-screen bg-black flex items-center justify-center">
				<Card className="bg-black/20 border-[#FF2D78]/20 w-96">
					<CardContent className="p-6">
						<div className="flex items-center justify-center">
							<div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#FF2D78]" />{" "}
	...
	return (
		<div className="min-h-screen bg-black">
			{/* Header */}
			<div className="bg-black/50 border-b border-[#FF2D78]/20 p-4">
				<div className="container mx-auto flex items-center justify-between">
					<div className="flex items-center gap-2 md:gap-4">
						<Button
							variant="ghost"
							onClick={() => router.push("/")}
							className="text-[#FF2D78] hover:bg-[#FF2D78]/10 p-2 md:p-2"
						>
							<ArrowLeft className="w-4 h-4 md:mr-2" />
							<span className="hidden md:inline">Back to Site</span>
						</Button>
						<div className="hidden md:block h-6 w-px bg-[#FF2D78]/30" />
						<h1 className="text-lg md:text-xl font-bold text-white">Admin Panel</h1>
						{userRole && (
							<span className="px-2 py-1 text-xs bg-[#FF2D78]/10 text-[#FF2D78] border border-[#FF2D78]/30 hidden sm:inline">
								{userRole}
							</span>
						)}
					</div>
					...
					<Button
						...
						className="md:hidden text-[#FF2D78] hover:bg-[#FF2D78]/10 p-2"
					>
			...
			{/* Mobile Navigation */}
			{mobileMenuOpen && (
				<div className="md:hidden bg-black/90 border-b border-[#FF2D78]/10">
			...
			{/* Desktop Navigation */}
			<div className="hidden md:block bg-black/30 border-b border-[#FF2D78]/10 p-4">

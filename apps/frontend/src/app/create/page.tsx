...
const PromptComponent = () => {
	...
	return (
		<div
			className="flex gap-4 bg-[#3333331A] px-4 items-center rounded-lg"
			style={{
		...
		>
			<textarea
				ref={textareaRef}
				className="w-full rounded-lg py-3 px-4 resize-none overflow-auto focus:outline-none bg-transparent text-white"
...
const AIImageWithPlaceHolder = ({ href }: { href: string | undefined }) => {
	...
	return (
		<div className="w-full h-full relative rounded-lg overflow-hidden">
			<Image alt="Generated Image" src={href} layout="fill" objectFit="cover" className="rounded-lg" />
		</div>
	);
};
					...
					<div>
						<div className="grid grid-cols-3 gap-4 mt-4">
							{nextImages.map((image, index) => {
								// Use image URL as key if available, otherwise fallback to index (not ideal, but necessary for undefined)
								const key = image ? image : `placeholder-${index}`;
								return (
									<button
										type="button"
										onClick={() => {
										...
										key={key}
										className="w-full aspect-square hover:cursor-pointer rounded-lg overflow-hidden relative"
									>
...
const TokenInfoInput = ({
	title,
	label,
	value,
	setValue,
}: { title: string; label?: string; value: string; setValue: (value: string) => void }) => {
	return (
		<div className="flex flex-col gap-2 w-full">
			<p className="text-xl font-[500]">{title}</p>
			<div
				className="flex items-center w-full px-4 gap-4 rounded-lg"
				style={{
					background: "linear-gradient(180deg, #171717 0%, #141414 100%)",
				}}
			>
			...
			<div className="inline-block">
				...
				<div className="flex gap-4 mt-4">
					...
					<button
						...
						className="border border-[#03FF24] rounded-lg hover:cursor-pointer px-4 py-2 w-full"
					>
						{" "}
						{/* Use flex-1 to share width */}
						<p>GENERATE</p>
					</button>
				</div>
			</div>
	...
	return (
		<div className="inline-block py-4">
			...
			<div className="flex gap-8 mt-4 items-center">
				...
				>
					<input className="rounded-lg focus:outline-none text-lg w-10" type="text" placeholder="0.00" />
		...
		<PromptProvider>
			<div className="flex justify-center">
				<div className="flex flex-col items-center mt-5 w-full max-w-[1100px]">
					<div>
						<img src="/create/coin-machine.png" alt="Coin machine illustration" />
					</div>

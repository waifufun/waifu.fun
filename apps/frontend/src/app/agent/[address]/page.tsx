...
async function buildAgentActivity(opts: {
	...
	const hasGithubScope = Boolean(opts.githubScope.login) || (opts.githubScope.repos?.length ?? 0) > 0;
...
export default async function AgentPage({
	...
	// Holdings: the aggregated /v2/agents/:address/holdings endpoint is the
	// ONLY source for this page. It is keyed by the requested agent, so it
	// can never surface another agent's (or the platform's) balances.
	//
	// When it returns null (404 / stale / not yet funded) we fall back to an
	// honest EMPTY snapshot, never to a hardcoded shared burner. A non-keyed
	// native-balance read would render the platform burner's money on this
	// agent's page, which is a launch-blocking integrity bug. The hero NAV
	// can still recover a real number from the agent's OWN AgentSafe balance
	// (keyed off the launch row, computed below).
	...
	// Holdings render strictly from the keyed aggregator. No aggregated
	// snapshot means an honest EMPTY allocation (the donut shows
	// "no priced assets yet") rather than a non-keyed burner read that
	// would leak the platform / another agent's balances onto this page.
	// The hero NAV recovers a real number only from the agent's OWN
	// AgentSafe balance (keyed off the launch row, see agentSafeBalance).

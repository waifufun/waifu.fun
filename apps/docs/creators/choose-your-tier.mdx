...
| BASED | TIER_90 | 32 BNB | 20 BNB | 12 BNB | on |
| WAGMI | TIER_95 | 64 BNB | 20 BNB | 44 BNB | on |
| GIGACHAD | TIER_98 | 160 BNB | 20 BNB | 140 BNB | on |
...
- **curve fill** is how much BNB the bundle pays to FLAP's `newTokenV6`. for
  ...
  at the four-fifths threshold (20 BNB). for SMOL (TIER_80) it is the full 16 BNB
  curve allocation, no graduation.
- **PCS follow-up buy** is BNB the bundle spends on the freshly-deployed PCS
  pair in the same atomic transaction. zero for SMOL, larger for higher tiers.

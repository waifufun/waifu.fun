...
once the wave H mainnet deploy is live.

{/* TODO verify deployed factory address once mainnet ships */}

## waifu.fun core (in scope)

| contract | address | notes |
| --- | --- | --- |
| `LaunchFactory` | TBD on mainnet | singleton entry point. immutable. |
| `LaunchVault` | per-launch | deployed by factory on `createLaunch`. one per launch. |
| `BundleRouter` | per-launch | deployed by factory on `createLaunch`. one per launch. |
| `TreasuryLP` | per-launch | deployed by factory on `createLaunch`. one per launch. |
...
`BundleRouter` clone. they are not configurable post-deploy.

## verifying a launch's contracts

every launch trio (vault, router, treasury) is deployed by the factory in
the same `createLaunch` transaction. you can verify on bscscan:
...
5. confirm the treasury's `factory()` returns the LaunchFactory

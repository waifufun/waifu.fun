...
a launch on waifu.fun is a per-token deployment with its own escrow, its own
atomic executor, its own treasury slot. one creator, one token, one set of
contracts. when it is done, those contracts are still around but they are
inert: vault distributes tokens to claimers, treasury holds 10%, router is
one-shot and locked.
...
3. if the curve graduates (tier 90+), PCS v2 pair gets created automatically
4. if `v2BuyBnb > 0`, follow-up buy from the PCS pair (post-tax)
5. read `balanceOf(router)`, split: 50% burn, 10% TreasuryLP, ~40% vault

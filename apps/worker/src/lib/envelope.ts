...
function parseKeyHex(keyRef: string, value: string | undefined): Buffer {
	if (!value) throw new Error(`Missing X token KEK for keyRef '${keyRef}'`);
...
function parseKekMap(): Map<string, Buffer> {
	...
	if (keyedEnv && keyedEnv.trim().length > 0) {
		...
		if (map.size === 0) throw new Error("X_TOKEN_KEKS_HEX did not contain any usable KEKs");
		return map;
	}
...
function getActiveKey(): { keyRef: string; key: Buffer } {
	...
	if (!keyRef) throw new Error("X_TOKEN_KEK_ACTIVE is required when X_TOKEN_KEKS_HEX is set");
	const key = keys.get(keyRef);
	if (!key) throw new Error(`Active X token KEK '${keyRef}' is not configured`);
	return { keyRef, key };
}

function getKeyForEnvelope(keyRef: string): Buffer {
	const key = parseKekMap().get(keyRef);
	if (!key) throw new Error(`X token KEK '${keyRef}' is not configured`);
	return key;
}

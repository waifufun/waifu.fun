...
`:id` accepts the token address (lowercased), the persona UUID, or a
slug for featured agents (e.g. `sol`).
...
```bash
curl "https://api.waifu.fun/v2/agents/sol/events?eventType=trade.open&limit=20"
```
...
| `POST /v2/webhooks/eliza-cloud/credits` | Eliza Cloud | `credits.topped_up`, `credits.deducted` |

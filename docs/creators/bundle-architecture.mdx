...
flowchart LR
  ...
  Router -- safeTransfer 100M --> Treasury[TreasuryLP5]
...
6. **split the token balance.** the router reads `totalY = balanceOf(this)`
   ...
   - `treasury = supply / 10` (= 10% of supply = 100M tokens, for TreasuryLP5)
...
7. **distribute the splits.** `safeTransfer` the vault share to `LaunchVault`,
   the treasury share to `TreasuryLP5`, the burn share to `0x...dEaD`. then
   call `vault.distribute(token, vaultAmt)` so claims can compute pro-rata,
   and `treasuryLp5.recordManagedToken(token)` to lock the launch token in
...
- 100M tokens (10% of supply) to `TreasuryLP5` for the V3 LP ladder
...
sitting in `TreasuryLP5`. the treasury cannot run its oracle yet because
...
2. call `treasuryLp5.setFlapV2Pair(pair)` to wire the TWAP oracle source
3. transfer ownership of the TreasuryLP5 from the factory to the agent's Safe
4. emit `LaunchFinalized`

after `finalizeLaunch`, the TreasuryLP5 is fully owned by the agent's Safe.

...
Status: partially fixed before this cleanup, now fully addressed here. Launch creation requires patron/agent auth plus SIWE verification; this cleanup adds depositor pagination metadata and makes single-address aggregation use a targeted uncapped query.

Findings:

- [P1] Require authorization before broadcasting launches - `apps/api/src/routes/v2/agent-launches.ts:169`
...
- [P2] Avoid truncating depositor aggregates - `apps/api/src/services/launch-v2/launch-repo.ts:151`

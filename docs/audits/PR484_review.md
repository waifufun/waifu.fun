...
Status: fixed by the Wave H security hardening branch. The current router pulls launch BNB from the vault during `executeBundle`, computes token splits from its actual balance, and transfers the presaler share to the vault before distribution is recorded.

Findings:

- [P1] Use the vault-funded balance when executing - `packages/contracts-evm/contracts/BundleRouter.sol:81`
...
- [P1] Don't strand curve-fill tokens in the router - `packages/contracts-evm/contracts/BundleRouter.sol:85`

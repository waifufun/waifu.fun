...
//      (0.1 BNB cap, 0.1 BNB quoteAmt, 0 BNB v2 buy, no vesting) budget
...
//   3. Token splits remain correct for TIER_TEST: vault = 25% of actual
//      curve tokens received, agentSafe = 12.5%, burn/noBurn = remainder.
//   4. The 0.1 BNB quoteAmt is intentionally below FLAP's production
//      graduation threshold; the proof target is cheap LAUNCHED state with no
//      BundleRouter InsufficientFunding revert.
//
// Run with:
...
describe("Wave H TIER_TEST + noBurn real-fork integration", function () {
	...
	it("[live] TIER_TEST + noBurn=true full launch + bundle + claim against real Portal", async () => {
		...
		// 4. Fill the 0.1 BNB cap, respecting 60% wallet cap (= 0.06 BNB).
		//    creator deposits 0.06 BNB; depositor2 deposits 0.04 BNB.
		const presaleCap = ethers.parseEther("0.1");
		const walletCap = (presaleCap * 6_000n) / 10_000n; // 0.06 BNB
		expect(await vault.presaleCap()).to.equal(presaleCap);

		const depositA = ethers.parseEther("0.06"); // at wallet cap
		const depositB = presaleCap - depositA; // = 0.04 BNB
		...
		const bundleEvent = execReceipt.logs
			.map((l) => {
				try {
		...
		expect(bundleEvent).to.not.equal(undefined);

		// (c) TIER_TEST vault share is proportional to actual curve tokens received.
		const vaultTokenBalance = await token.balanceOf(launchAddrs.vault);
		expect(vaultTokenBalance).to.equal(bundleEvent.args.tokensToVault);
		expect(vaultTokenBalance).to.equal(bundleEvent.args.tokensReceived / 4n);

		// (d) AgentSafe share is proportional too. TreasuryLP5 is dormant before
		//     deferred V3 activation (post-#672 behavior).
		...
		expect(agentSafeTokenBalance).to.equal(bundleEvent.args.tokensToTreasury);
		expect(agentSafeTokenBalance).to.equal(bundleEvent.args.tokensReceived / 8n);
		...
		//     Splits: vault=25% of tokens received, agentSafe=12.5%,
		...
		// (g) 0.1 BNB is intentionally below FLAP's V2 migration threshold.
		//     The cheap smoke lane proves vault LAUNCHED + proportional distribution,
		//     not production-priced V2 migration.
		...
		console.log(`    [tier-test] PCS V2 pair at ${pair} (expected zero for cheap curve-only smoke)`);

		// (h) Vault state == LAUNCHED (enum value 2).
		expect(await vault.state()).to.equal(2);

		// (i) The router distributed its complete token balance across vault,
		//     agentSafe, and noBurn destination.
		const accounted = vaultTokenBalance + agentSafeTokenBalance + treasuryTokenBalance + creatorTokenBalance;
		expect(accounted).to.equal(bundleEvent.args.tokensReceived);
		console.log(
			`    [tier-test] proportional split: received=${ethers.formatUnits(bundleEvent.args.tokensReceived, 18)} vault=${ethers.formatUnits(vaultTokenBalance, 18)} agentSafe=${ethers.formatUnits(agentSafeTokenBalance, 18)} creator=${ethers.formatUnits(creatorTokenBalance, 18)}`,
		);
		...
		console.log("    presaleCap:         0.1 BNB (0.06 + 0.04)");
		console.log("    quoteAmt:           0.1 BNB");
		...
		console.log(`    V2 pair:            ${pair} (zero expected for cheap curve-only smoke)`);
		...
		console.log(`    vault balance:      ${ethers.formatUnits(vaultTokenBalance, 18)} (25% of received)`);
		console.log(`    agentSafe balance:  ${ethers.formatUnits(agentSafeTokenBalance, 18)} (12.5% of received)`);
		console.log(
			`    treasuryLp balance: ${ethers.formatUnits(treasuryTokenBalance, 18)} (deferred V3, 0 pre-finalize)`,
		);
		console.log("    pair token reserve: n/a (curve-only smoke)");
		console.log("    ===============================================\n");
	});
});

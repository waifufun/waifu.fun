...
describe("WaifuFun Security Regression Tests", () => {
	beforeEach(async function () {
		...
		this.tokenAddress = (await this.WaifuFun.getLaunchedTokensByOwner(this.user_1.address))[0];
		this.token = new ethers.Contract(this.tokenAddress, ERC20ABI, this.deployer);
	});
	...
	it("reverts dust buys that would otherwise produce zero output", async function () {
		...
		const dustBuyToken = (await this.WaifuFun.getLaunchedTokensByOwner(this.user_2.address))[0];
		const beforeCurve = await this.WaifuFun.bondingCurvesByToken(dustBuyToken);
		...
		const afterCurve = await this.WaifuFun.bondingCurvesByToken(dustBuyToken);
		expect(afterCurve.reserveETHAmount).to.equal(beforeCurve.reserveETHAmount);
		expect(afterCurve.reserveTokenAmount).to.equal(beforeCurve.reserveTokenAmount);
	});

...
const CLAIM_REWARDS_SELECTOR = "0x372500ab"; // claimRewards()
...
describe("AgentSafeZodiacDeployer", () => {
	...
	beforeEach(async () => {
		...
		actionTarget = await ActionTarget.deploy();
	...
	async function deploySafeWithRoles() {
		...
		const receipt = await (
			await zodiacDeployer.deployAgentSafeWithRoles(owners, threshold, safeSalt, rolesSalt, configCalls)
		).wait();
	...
	it("lets agent-hot execute the default allowed action through Roles", async () => {
		...
		const agentRole = await zodiacDeployer.AGENT_ROLE();
		const data = actionTarget.interface.encodeFunctionData("claimRewards");
...
describeFork("AgentSafeZodiacDeployer :: BSC fork Safe v1.4.1 module setup", () => {
	it("enables the Roles module on a canonical Safe proxy during setup", async () => {
		...
		await zodiacDeployer.deployAgentSafeWithRoles(owners, 2, 20260609n, 20260610n, configCalls);

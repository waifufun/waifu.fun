...
describe("WaifuFun Contract Deployment", () => {
	...
	it("keeps the expected owner and global config", async function () {
		...
		expect(onChainConfig.buyFee.toString()).to.equal(this.globalConfig.buyFee.toString());
		expect(onChainConfig.sellFee.toString()).to.equal(this.globalConfig.sellFee.toString());
	});
});

 ...
 * mc-to-tick.js
 *
 * Pure-JS helper to derive TreasuryLP5 treasury-tick ranges from market-cap
 * targets in USD.
 *
 * Background:
 *   Wave O.1 TreasuryLP5 dropped the on-chain MC ladder (no Chainlink,
 *   no oraclePoke, no targetMcUSD). Tier activation is now driven by
 *   V3 price-band tick ranges that the CALLER computes off-chain at
 *   create-launch time. This module is the single source of truth for
 *   that computation.
 *
 *   For each MC target, we compute the tick offset from the launch tick
 *   such that the V3 price at `launchTick + offset` corresponds to a
 *   token-per-WBNB price implying the target FDV. Because PCS V3 1%-fee
 *   pools use tickSpacing = 200, we round to the nearest 200-multiple.
 *   Upper ticks are pinned to MAX_TICK_PCS_V3_1PCT (887200) so each tier
 *   is single-sided WBNB at launch (no token).
 *
 * Conventions:
 *   - Tick direction: a higher tick == higher WBNB-per-token == higher
 *     token-implied price in USD. So tier-N lower = launchTick + positive
 *     offset for any mcTarget > launchFdvUsd.
 *   - PCS V3 1% fee tier: tickSpacing = 200, MAX_TICK = 887200.
 *   - We use Math.round (not floor/ceil) so the activation price stays
 *     symmetric around the requested MC target. Solidity-side
 *     `_floorToSpacing` further floor-aligns at mint time if needed,
 *     so callers should pass already-aligned ticks.
 *
 * Usage:
 *   const { computeTreasuryTicksFromMc, WAGMI_MC_TARGETS_USD } =
 *     require("./mc-to-tick.js");
 *   const launchTick = await deriveLaunchTickFromV2Reserves(pair);
 *   const launchFdvUsd = computeLaunchFdvUsd(reserves, bnbUsd, supply);
 *   const { lowers, uppers } =
 *     computeTreasuryTicksFromMc(launchTick, launchFdvUsd);
 *
 *   // pass lowers/uppers to LaunchFactory.createLaunch via the config.
 */

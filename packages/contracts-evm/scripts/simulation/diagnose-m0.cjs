...
async function main() {
	...
	const token = new ethers.Contract(mined.predicted, [
	...
	const wbnb = new ethers.Contract(BSC.WBNB, ["function balanceOf(address) view returns (uint256)", "function deposit() payable"], ethers.provider);
	...
	const treasuryAsAgent = new ethers.Contract(finalLaunches.treasuryLp, ["function setEpochLength(uint256)"], await ethers.getSigner(launches.agentSafe));
	...
	log(`after ${attempts} attempts: epochsAbove=${tier0After.epochsAbove}/${tier0After.minEpochs}, deployed=${tier0After.deployed}`);
	...
	log(`antiFarmerExpirationT  = ${afExp}  (${afExp - BigInt(now)} sec remaining, ${afExp > BigInt(now) ? "ACTIVE" : "EXPIRED"})`);
	...
	const v3FactoryC = new ethers.Contract(BSC.PCS_V3_FACTORY, ["function getPool(address,address,uint24) view returns (address)"], ethers.provider);
	...
	function predictV3Pool(token0, token1, fee) {
		...
		const hashedSalt = ethers.keccak256(ethers.AbiCoder.defaultAbiCoder().encode(["address", "address", "uint24"], [token0, token1, fee]));
	...
	log(`Treasury -> V2 pair: sent 1000, received ${ethers.formatUnits(received1, 18)} (taxed=${received1 < ethers.parseUnits("1000", 18)})`);
	...
	if (v3PoolAddr === ethers.ZeroAddress) {
		...
		const npmIface = new ethers.Contract(BSC.PCS_V3_NPM, [
	...
	log(`Treasury -> V3 pool: sent 1000, received ${ethers.formatUnits(received2, 18)} (taxed=${received2 < ethers.parseUnits("1000", 18)})`);
	...
	log(`Treasury -> NPM:      sent 1000, received ${ethers.formatUnits(received3, 18)} (taxed=${received3 < ethers.parseUnits("1000", 18)})`);
	...
	log(`Treasury -> EOA:      sent 1000, received ${ethers.formatUnits(received4, 18)} (taxed=${received4 < ethers.parseUnits("1000", 18)})`);

...
//   6. finalizeLaunch deploys TreasuryLP4 and seeds the V3 pool
...
const SUKI = {
	...
	noBurn: false,
	// Wave O.0 infinity-tier config: tier entry tick derived from MC ladder
	// at $600 BNB / 1B supply (matches infinity-tier-pressure golden numbers
	// for MC checkpoints $5M / $10M / $25M / $100M).
	treasuryTickLowers: [2000, 9000, 18200, 32000],
	treasuryTickUppers: [887200, 887200, 887200, 887200], // MAX_TICK 1% fee tier
...
async function main() {
	...
	// -----------------------------------------------------------------
	banner(2, "Deploy factory + helpers (Wave N path)");
	let factoryAddress;
	let factory;
	let mockFeed;
	let mockFeedAddress;
	try {
		...
		// Wave N gotcha: TreasuryLP4 calls Chainlink BNB/USD and reverts on
		// stale data. The fork's timestamp lags real wall-clock, so the real
		// mainnet feed at 0x0567...aeE looks stale after any evm_increaseTime
		// jump. We swap in a MockBnbUsdFeed at $600 for the dry-run only;
		// every other component (PCS V2, PCS V3, WBNB, Safe, FLAP portal) is
		// the real forked contract. The real factory deploy points at the
		// real Chainlink feed; on live mainnet, the feed updates every block.
		const MockFeed = await ethers.getContractFactory("MockBnbUsdFeed", deployer);
		mockFeed = await MockFeed.deploy(BigInt(BNB_USD_PRICE) * 100000000n);
		await mockFeed.waitForDeployment();
		mockFeedAddress = await mockFeed.getAddress();
		kv("MockBnbUsdFeed (test substitution)", mockFeedAddress);
		...
		const TLP4Dep = await ethers.getContractFactory("TreasuryLP4Deployer", deployer);
		const treasuryLp4Deployer = await TLP4Dep.deploy();
		...
		factory = await LF.deploy(
			...
			BSC.PCS_V3_NPM,
			BSC.PCS_V3_FACTORY,
			mockFeedAddress, // test substitution; real mainnet deploy uses BSC.BNB_USD_FEED
		);
		...
		const checks = [
			...
			["BNB_USD_FEED", await factory.BNB_USD_FEED(), mockFeedAddress],
		];
		let allOk = true;
	...
	try {
		...
		const config = {
			...
			treasuryTickLowers: SUKI.treasuryTickLowers,
			treasuryTickUppers: SUKI.treasuryTickUppers,
		};
		...
		kv("TreasuryLP4 (placeholder pre-finalize)", launches.treasuryLp);
	...
	// -----------------------------------------------------------------
	banner(8, "Snapshot post-bundle + finalizeLaunch -> TreasuryLP4");
	...
	try {
		...
		kv("TreasuryLP4 (post-finalize)", finalLaunches.treasuryLp);
		realNumbers.treasuryLp4Address = finalLaunches.treasuryLp;
		if (finalLaunches.treasuryLp === ethers.ZeroAddress) throw new Error("TreasuryLP4 not deployed");
		...
		kv("TreasuryLP4 token balance", tok(treasuryTokenBalance));
		realNumbers.treasuryLp4TokenBalanceAtFinalize = ethers.formatUnits(treasuryTokenBalance, 18);
		// Spec: TreasuryLP4 receives 100M tokens (10% of 1B supply) for tier deployment.
		// Allow 5% tolerance because supply could vary by version.
		const expected = ethers.parseUnits("100000000", 18);
		const delta = treasuryTokenBalance > expected ? treasuryTokenBalance - expected : expected - treasuryTokenBalance;
		const tolerance = expected / 20n; // 5%
		const within = delta <= tolerance;
		kv("TreasuryLP4 ~100M tokens", within ? "ok" : `OFF by ${tok(delta)}`);
		...
		const verdictBits = [
			...
			`TreasuryLP4 ${finalLaunches.treasuryLp} holds ${tok(treasuryTokenBalance)}`,
			`${tok(totalPresaler)} delivered to 8 presalers`,
		];
		if (!within) {
			investigate(8, `${verdictBits.join("; ")} (TreasuryLP4 balance off from 100M spec)`);
		} else {
	...
	// -----------------------------------------------------------------
	// STEP 9: PCS V2 buys to push price through tier 0 entry
	// -----------------------------------------------------------------
	banner(9, "PCS V2 buys to clear tier 0 ($5M MC) on V2 leg");
	...
	try {
		// Anti-farmer window must expire before V2 buys.
		await increase(SUKI.antiFarmerDuration + 60);
		const buyTraders = [t1, t2, t3, t4, t5];
		// V2 ladder to push above $5M MC: with seed of 47 BNB and ~900M tokens,
		// MC starts at roughly $30k. We need to pump ~165x to hit $5M.
		const ladder = [5, 10, 20, 40, 100];
		...
		const finalSnap = await pairSnapshotMc(pair, token, wbnb);
		realNumbers.v2McAfterPumpUsd = `$${finalSnap.mcUsd.toFixed(0)}`;
		endBanner();
		if (finalSnap.mcUsd >= SUKI.mcCheckpoints[0]) {
			pass(9, `V2 MC pumped to $${finalSnap.mcUsd.toFixed(0)} (clears tier 0 $5M)`);
		} else {
			investigate(9, `V2 MC only reached $${finalSnap.mcUsd.toFixed(0)}, did not clear $5M tier 0 entry`);
		}
	} catch (e) {
		endBanner();
		fail(9, `V2 pump failed: ${e.shortMessage || e.message}`);
		throw e;
	}

	// -----------------------------------------------------------------
	// STEP 10: Treasury epoch -> deploy tier 0
	// -----------------------------------------------------------------
	banner(10, "Advance epochs and deploy tier 0 (overlap [2000, 887200])");
	...
	let tier0PositionId;
	try {
		// Shrink epoch length so we can advance quickly. Only AgentSafe can call.
		...
		const tier0Spec = await treasury.tiers(0);
		const minEpochs = Number(tier0Spec.minEpochs);
		kv("tier 0 minEpochs", minEpochs.toString());
		kv("tier 0 targetMcUSD", tier0Spec.targetMcUSD.toString());
		let safety = 0;
		while ((await treasury.tiers(0)).epochsAbove < BigInt(minEpochs)) {
			await increase(3600);
			await refreshFeed(mockFeed);
			await (await treasury.checkAndAdvance()).wait();
			safety += 1;
			kv(`epoch ${safety} epochsAbove`, (await treasury.tiers(0)).epochsAbove.toString());
			if (safety > 12) throw new Error("tier 0 did not advance after 12 epochs");
		}
		const tier0Final = await treasury.tiers(0);
		kv("tier 0 deployed", tier0Final.deployed.toString());
		kv("tier 0 positionId", tier0Final.positionId.toString());
		kv("tier 0 ticks", `[${tier0Final.tickLower}, ${tier0Final.tickUpper}]`);
		tier0PositionId = tier0Final.positionId;
		realNumbers.tier0PositionId = tier0Final.positionId.toString();
		realNumbers.tier0Ticks = `[${tier0Final.tickLower}, ${tier0Final.tickUpper}]`;
		if (!tier0Final.deployed || tier0Final.positionId === 0n) {
			throw new Error("tier 0 deploy did not produce a position id");
		}
		if (Number(tier0Final.tickUpper) !== MAX_TICK) {
			throw new Error(`tier 0 upper tick ${tier0Final.tickUpper} != MAX_TICK ${MAX_TICK}`);
		}
		const pos = await npm.positions(tier0PositionId);
		kv("V3 position liquidity", pos.liquidity.toString());

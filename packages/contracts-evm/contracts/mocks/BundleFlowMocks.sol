...
contract BundleFlowToken is ERC20 {
    ...
    function adminSetTransferTaxBps(uint16 bps) external {
        require(msg.sender == minter, "BundleFlowToken: not minter");
        require(bps <= 10_000, "BundleFlowToken: bad bps");
        transferTaxBps = bps;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        uint256 tax = (amount * transferTaxBps) / 10_000;
        if (tax > 0 && to != DEAD) {
            super._transfer(from, DEAD, tax);
...
contract ERC20FeeMock is ERC20 {
    ...
    constructor() ERC20("Fee Mock", "FEE") {}

    function mint(address to, uint256 amount) external {
        _mint(to, amount);
    }

    function setTransferTaxBps(uint16 bps) external {
        require(bps <= 10_000, "ERC20FeeMock: bad bps");
        transferTaxBps = bps;
    }
...
contract MockFlapPortalCREATE2 {
    ...
    uint16 public tokenTransferTaxBps;
    ...
    function setTokenTransferTaxBps(uint16 bps) external {
        tokenTransferTaxBps = bps;
    }
    ...
    function newTokenV6(FlapTypes.NewTokenV6Params calldata params) external payable returns (address token) {
        ...
        if (useReentrantToken) {
            bytecode =
                abi.encodePacked(type(BundleFlowTokenReentrant).creationCode, abi.encode(params.name, params.symbol));
        } else {
            bytecode = abi.encodePacked(type(BundleFlowToken).creationCode, abi.encode(params.name, params.symbol));
        }
        ...
        if (useReentrantToken) {
            ...
            BundleFlowToken(token).adminSetTransferTaxBps(tokenTransferTaxBps);

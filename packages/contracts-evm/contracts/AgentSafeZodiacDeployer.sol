...
/// @notice AgentSafe deployer that atomically creates a Safe, attaches a
///         Zodiac Roles-compatible module, and applies the initial constrained
///         role configuration for the agent-hot EOA.
/// @dev    Role config is raw calldata so the off-chain agent-actions encoder
///         can own the exact Roles v2 ABI. Empty role config is allowed only for
///         pending-agent launches: the Safe is still born with the module
///         enabled, but no agent EOA has membership until owners configure it.

...
async function setupWaifuFunSuite() {
    const [deployer, user_1, user_2, fallbackTeamWallet] =
        await ethers.getSigners();

    const deployment = parseDeployment();

    if (deployment) {
        const WaifuFun = await ethers.getContractAt(
            "WaifuFun",
            deployment.waifuFun,
            deployer
        );
            ...
            [deployer, user_1, user_2, fallbackTeamWallet].find(
                (signer) =>
                    ...
                    signer.address.toLowerCase() ===
                        globalConfig.teamWallet.toLowerCase()
            ) || fallbackTeamWallet;
    ...
    const WaifuFunTokenFactory = await deployProxy(
        "WaifuFunTokenFactory",
        "WaifuFunTokenFactory"
    );

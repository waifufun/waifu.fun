...
describe("VeWaifuStaking", function () {
  let waifu, staking, owner, alice, bob;
  ...
  it("should stake WAIFU", async function () {
  ...
  it("should withdraw WAIFU", async function () {
    await staking.connect(alice).stake(STAKE_AMOUNT);
  ...
  it("should exit (withdraw all + claim)", async function () {
    await staking.connect(alice).stake(STAKE_AMOUNT);
    await waifu.transfer(staking.address, REWARD_AMOUNT);
    await staking.notifyRewardAmount(REWARD_AMOUNT);
  ...
  it("should revert notifyRewardAmount from non-distributor", async function () {
    await staking.connect(alice).stake(STAKE_AMOUNT);
    await expect(staking.connect(alice).notifyRewardAmount(REWARD_AMOUNT))
      .to.be.reverted;
  });

  it("should revert notifyRewardAmount with no stakers", async function () {

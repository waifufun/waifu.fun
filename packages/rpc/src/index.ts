...
import {
  CHAINID_TO_VIEM_CHAIN,
  EVM_RPC_URLS,
  SOLANA_RPC_URLS,
} from "@autofun/constants";
...
type Erc20FunctionName = ReadContractParameters<
  typeof erc20Abi
>["functionName"];
...
export class SolanaRpcProvider {
  ...
  static async connect(
  ...
  ): Promise<SolanaRpcProvider> {
    if (
      SolanaRpcProvider.currentRpc &&
      SolanaRpcProvider.currentRpc.networkId === networkId
    ) {
    ...
    while (attempts < maxAttempts) {
      ...
      if (!rpc) {
        throw new Error(
          `No RPC URL found at index ${SolanaRpcProvider.currentRpcIndex} for Solana: ${networkId}`
        );
      }
      try {
  ...
  getTokenMetadata = withFallBack(async (contractAddress: string) => {
    ...
    const uriData = (await fetch(uri).then(
  ...
  private getTokenSupplies = withFallBack(
    async (mintAddresses: PublicKey[]) => {
  ...
  getBondingCurveInfo = withFallBack(async (contractAddresses: string[]) => {
    ...
    const tokenMints: PublicKey[] = contractAddresses.map(
    ...
    if (!solanaUsdPrice)
      throw new Error("Unable to determine Solana USD price");
    ...
    return bondingCurves.map((curve, i) => {
      ...
      if (!supplyInfo)
        throw new Error(`Unable to determine supplyInfo for token: ${mint}`);

      if (
        !curve ||
        !curve.reserveToken ||
        curve.reserveToken.toNumber() === 0
      ) {
        return {
          tokenMint: mint,
          curveCompleted: null,
      ...
      const priceSOL =
        reserveLamport / 1e9 / (reserveToken / 10 ** tokenDecimals);
  ...
  getTokenBalance = withFallBack(
    async (contractAddress: AddressLike, owner: AddressLike, raw?: boolean) => {
      const mint = new PublicKey(contractAddress);
      ...
      const decimals =
        accountInfo.value?.data && "parsed" in accountInfo.value.data
          ? accountInfo.value.data.parsed.info.decimals
          : 6;
  ...
  getBlock = withFallBack(
    async (blockNumber: number): Promise<VersionedBlockResponse | null> => {

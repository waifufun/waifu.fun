...
schema.index(
  { signature: 1, instructionIndex: 1, programId: 1 },
  { unique: true, name: "unique_instruction" }
);
...
schema.statics.createOrUpdate = async function (
  ...
  try {
    ...
    if (error?.code === 11000) {
      ...
      logger.info(
        `Event already exists: ${eventData.signature}:${eventData.instructionIndex}`
      );
...
schema.statics.insertManyOrUpdate = async function (
  ...
  try {
    ...
    logger.info(
      `Bulk operation completed: ${result.upsertedCount} inserted, ${result.modifiedCount} updated`
    );

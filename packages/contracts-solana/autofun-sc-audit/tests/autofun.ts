...
(isSolanaAuditIntegrationEnabled() ? describe : describe.skip)(
  ...
  () => {
    ...
    it("correctly configured", async () => {
      ...
      assert.equal(
        configAccount.tokenSupplyConfig.range.min?.toString(),
        "5000"
      );
      assert.equal(
      ...
      assert.equal(
        configAccount.initBondingCurve,
        TEST_CONFIG.initBondingCurve
      );
    });
    ...
    it("token created", async () => {
      ...
      const curveShare =
        (BigInt(TEST_CONFIG.tokenSupply) *
          BigInt(TEST_CONFIG.initBondingCurve)) /
        100n;

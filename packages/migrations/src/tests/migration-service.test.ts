...
//     { name: "step1", execute: sinon.stub().resolves({ success: true }) },
//     { name: "step2", execute: sinon.stub().resolves({ success: true }) },
//     { name: "step3", execute: sinon.stub().resolves({ success: true }) },
//     { name: "step4", execute: sinon.stub().resolves({ success: true }) },
//   ]);
...
//   // Use status "migrating" initially
//   const mockMigration: IMigration = {
//     _id: "test-migration-id",
//     protocol: "raydium",
//     status: "migrating",
//     currentStep: 0,
...
//   describe("Raydium Migration Scenarios", () => {
...
//       const stepFourUpdate = allSets.find((set) => set.currentStep === 4);
//       expect(stepFourUpdate).to.exist;
//       expect(stepFourUpdate!.lastSuccessfulStep).to.equal(3);
//       expect(stepFourUpdate!.lastProcessedAt).to.exist;
...
//       expect(errorUpdate!.errors).to.equal("Transaction failed");
...
//     it("should successfully complete a migration with all steps (finalize immediately)", async () => {
//       mockMigrationManager.getMigrationSteps.resolves([
//         { name: "step1", execute: sinon.stub().resolves({ success: true }) },
//         { name: "step2", execute: sinon.stub().resolves({ success: true }) },
//         { name: "step3", execute: sinon.stub().resolves({ success: true }) },
...
//       mockDb.Migration.find.returns({
//         limit: sinon.stub().resolves([
//           { ...mockMigration, _id: "id1", status: "migrating" },
//           { ...mockMigration, _id: "id2", status: "migrating" },
//         ]),
//       });

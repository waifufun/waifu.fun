...
export async function createPool(
	...
	try {
		...
		// Get token info
		const mintA = await raydium.token.getTokenInfo(new PublicKey(params.tokenMint));
...
export async function initRaydiumSdkAndFetchPoolInfo(
	...
	try {
		...
		await recordTransaction(context.state, "initLockLP", state.txId || "init-lock-lp", {
		...
		console.error("Error initializing Raydium SDK and fetching pool info:", error);
		throw error;
	}
}

export async function lockLP({ context, poolId, amount, isPrimary }: LockLPParams): Promise<string> {
	const { rpc, wallet, state } = context;
	if (!wallet) {
	...
	try {
		...
		await recordTransaction(state, isPrimary ? "lockPrimaryLP" : "lockSecondaryLP", txId, {
		...
		await DB.Migration.findOneAndUpdate(
			...
			{
				$set: {
					[`${isPrimary ? "primary" : "secondary"}LockAmount`]: amount.toString(),
		...
		console.error(`Error locking ${isPrimary ? "primary" : "secondary"} LP:`, error);
		throw error;
	}
}
...
export async function sendNftToManagerMultisig(
	...
	try {
		...
		const signerTokenAccount = spl.getAssociatedTokenAddressSync(new PublicKey(nftMint), wallet.publicKey);
		const multisigTokenAccount = spl.getAssociatedTokenAddressSync(new PublicKey(nftMint), multisigAddress);
...
export async function finalizeMigration(context: MigrationContext): Promise<void> {
	const { state } = context;

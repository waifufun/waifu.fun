...
export const raydiumMigrationSteps: MigrationStep[] = [
  ...
  },
  {
    ...
    execute: async (context: RaydiumMigrationContext) => {
      ...
      if (!state.withdrawnAmounts) {
        throw new Error(
          "No withdrawn amounts found. Withdrawal must be completed first."
        );
      }
  ...
  },
  {
    ...
    execute: async (context: RaydiumMigrationContext) => {
      ...
      if (!state.txId) {
        throw new Error(
          "No pool creation transaction found. Pool must be created first."
        );
      }
      ...
      const { raydium, poolInfo } = await initRaydiumSdkAndFetchPoolInfo(
        context,
        lastTx.data.poolAddresses.id
      );
      ...
      console.log(
        "LP token lock initialized for pool:",
        state?.poolAddresses?.id
      );
    },
    rollback: async (context: RaydiumMigrationContext) => {
      throw new Error("Not implemented");
  ...
  commonSendNftStep,
  {
    ...
    execute: async (context: RaydiumMigrationContext) => {
      ...
      const { txId } = await depositNftToRaydiumVault(
        context,
        nftMint,
        claimerAddress
      );

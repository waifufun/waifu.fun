...
export class DataSync {
	...
	private async transformToken(pgToken: Token): Promise<IToken> {
		...
		return {
			...
			image: (pgToken.image as TURLLike) || defaultImage,
			...
			bondingCurveAddress: (pgToken.marketId as AddressLike) || ("" as AddressLike),
			...
			socials: {
				twitter: (pgToken.twitter as TURLLike) || undefined,
				website: (pgToken.website as TURLLike) || undefined,
				discord: (pgToken.discord as TURLLike) || undefined,
				telegram: (pgToken.telegram as TURLLike) || undefined,
			},
	...
	async syncTokens(batchSize = 100): Promise<void> {
		try {
			...
			const cleanupResult = await this.mongoDb.Token.deleteMany({
				$or: [{ contractAddress: { $exists: false } }, { contractAddress: null }, { contractAddress: "" }],
			});

...
describe("AutoFun End-To-End Test", function () {
    let globalConfig;
    let BONDING_CURVE_FIXED_POINT = 1000000;
    let FEE_FIXED_POINT = 10000;
    before(async function () {
        [this.deployer, this.user_1, this.user_2, this.teamWallet] =
            await ethers.getSigners();
        this.AutoFunTokenFactory = await deployProxy(
            "AutoFunTokenFactory",
            "AutoFunTokenFactory"
        );
        globalConfig = getDeploymentParams();
        globalConfig.teamWallet = this.teamWallet.address;
        this.AutoFun = await deployProxy("AutoFun", "AutoFun", [globalConfig]);

        // transfer AutoFunTokenFactory's ownership to AutoFun contract
        await this.AutoFunTokenFactory.transferOwnership(this.AutoFun.address);
    });
    ...
    it("set AutoFunTokenFactory address", async function () {
        await expect(
            this.AutoFun.connect(this.teamWallet).updateFactory(
                this.AutoFunTokenFactory.address
            )
        ).to.be.revertedWith("Ownable: caller is not the owner");
        await this.AutoFun.updateFactory(this.AutoFunTokenFactory.address);
        expect(await this.AutoFun.factory()).to.be.equal(
            this.AutoFunTokenFactory.address
        );
    });
    ...
    it("update Global config", async function () {
        ...
        expect((await this.AutoFun.globalConfig()).maxDecimal).to.be.equal(18);
        await expect(
            this.AutoFun.connect(this.teamWallet).updateGlobalConfig(
                globalConfig
            )
        ).to.be.revertedWith("Ownable: caller is not the owner");
        await this.AutoFun.updateGlobalConfig(globalConfig);
        expect((await this.AutoFun.globalConfig()).maxDecimal).to.be.equal(20);
    });
    ...
    it("launch token and check", async function () {
        ...
        let symbol = "AFT_1";
        await this.AutoFun.connect(this.user_1).launch(
            BigInt(totalSupply),
        ...
        let allLaunchedTokens = await this.AutoFun.getAllLaunchedTokens();
        let launchedTokensByOwner = await this.AutoFun.getLaunchedTokensByOwner(
            this.user_1.address
        );
        ...
        let bondingCurve = await this.AutoFun.bondingCurvesByToken(
            launchedTokenAddress
        );
    ...
    describe("swap tokens and check", async function () {
        ...
        it("buy tokens and check", async function () {
            // swap ETH to Token
            tokenAddress = (
                await this.AutoFun.getLaunchedTokensByOwner(this.user_1.address)
            )[0];
            ...
            let beforeBondingCurve = await this.AutoFun.bondingCurvesByToken(
                tokenAddress
            );
            ...
            await this.AutoFun.connect(this.user_1).swap(
                {
                    token: tokenAddress,
            ...
            let afterBondingCurve = await this.AutoFun.bondingCurvesByToken(
                tokenAddress
            );
        ...
        it("sell tokens and check", async function () {
            ...
            await token
                .connect(this.user_1)
                .approve(this.AutoFun.address, BigInt(tokenAmountToSwap));
    ...
    describe("launch and swap token", function () {
        it("launch and swap token", async function () {
            ...
            await this.AutoFun.connect(this.user_2).launchAndSwap(
                BigInt(totalSupply),
                BigInt(virtualReserveETHAmount),
                decimals,
                name,
            ...
            let tokenAddress = (
                await this.AutoFun.getLaunchedTokensByOwner(this.user_2.address)
            )[0];
            let token = new ethers.Contract(
                tokenAddress,
                ERC20ABI,
    ...
    describe("withdraw", async function () {
        it("withdraw and check", async function () {
            ...
            let bondingCurve0 = await this.AutoFun.bondingCurvesByToken(
                tokenAddress0
            );
            let bondingCurve1 = await this.AutoFun.bondingCurvesByToken(
                tokenAddress1
            );
            ...
            await expect(
                this.AutoFun.connect(this.user_1).withdraw(tokenAddress0)
            ).to.be.revertedWith("Ownable: caller is not the owner");
            await expect(
                this.AutoFun.withdraw(tokenAddress1)
            ...
            await this.AutoFun.withdraw(tokenAddress0);

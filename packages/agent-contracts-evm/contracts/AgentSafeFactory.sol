...
contract AgentSafeFactory {
    ...
    event AgentSafeDeployed(
        address safe, address rolesModifier, address agentKey, address patronWallet
    );
    ...
    constructor(
        ...
        if (
            safeSingleton_ == address(0) || safeProxyFactory_ == address(0)
                || safeFallbackHandler_ == address(0) || rolesModifierSingleton_ == address(0)
        ) revert ZeroAddress();
        ...
        returns (address safe, address rolesModifier)
    {
        ...
        safe = ISafeProxyFactory(safeProxyFactory).createProxyWithNonce(
            safeSingleton, initializer, uint256(salt)
        );

        // Zodiac Roles Modifier setup params are owner/avatar/target. The Safe owns and executes through it.
        // TODO verify exact v1 initializer ABI before mainnet.

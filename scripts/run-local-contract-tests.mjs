...
const tempRoot = await mkdtemp(
  path.join(os.tmpdir(), "waifufun-local-contract-tests-")
);
const solanaLedgerDir = path.join(tempRoot, "solana-ledger");
const solanaWalletPath = path.join(tempRoot, "anchor-wallet.json");
const solanaAuditDir = path.join(
  rootDir,
  "packages",
  "contracts-solana",
  "autofun-sc-audit"
);
const solanaProgramLibPath = path.join(
  solanaAuditDir,
  "programs",
  "autofun",
  "src",
...
const solanaProgramIdlPath = path.join(
  solanaAuditDir,
  "target",
  "idl",
  "autofun.json"
);
const solanaProgramKeypairPath = path.join(
  solanaAuditDir,
  "target",
  "deploy",
  "autofun-keypair.json"
);
  ...
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      ...
      const error = new Error(
        `Command failed (${code}): ${formatCommand(command, args)}`
      );
      error.stdout = stdout;
      error.stderr = stderr;
      reject(error);
...
async function rewriteDeclaredProgramId(programId, canonicalProgramId) {
  ...
  if (currentProgramId === programId) {
    if (currentProgramId !== canonicalProgramId) {
      fileRestores.push(() =>
        writeFile(
          solanaProgramLibPath,
          replaceDeclaredProgramId(originalSource, canonicalProgramId)
        )
      );
    }
    return;
...
try {
  ...
  await runCommand("pnpm", ["--filter", "@waifufun/contracts-evm", "compile"], {
    env: evmEnv,
  });

  console.log("Deploying EVM contracts to anvil");
  const evmDeploy = await runCommand(
    "pnpm",
    ["--filter", "@waifufun/contracts-evm", "deploy:local"],
    {
      env: {
        ...evmEnv,
        PRINT_JSON: "1",
  ...
  await runCommand(
    "pnpm",
    ["--filter", "@waifufun/contracts-solana-audit", "build:local"],
    {
      env: solanaEnv,
    }
  );
  ...
  const solanaProgramAddress = extractSingleLine(
    (
      await runCommand(
        "solana",
        ["address", "-k", solanaProgramKeypairPath],
        {
          cwd: solanaAuditDir,
          env: solanaEnv,
        }
  ...
  await runCommand(
    "pnpm",
    ["--filter", "@waifufun/contracts-evm", "test:contracts:local"],
    {
      env: evmTestEnv,
    }
  );
  ...
  await runCommand(
    "pnpm",
    ["--filter", "@waifufun/contracts-solana-audit", "test:contracts"],
    {
      env: solanaTestEnv,
    }
  );
  ...
  await runCommand(
    "pnpm",
    ["--filter", "@waifufun/contracts-evm", "test:e2e:local"],
    {
      env: evmTestEnv,
    }
  );
  ...
  await runCommand(
    "pnpm",
    ["--filter", "@waifufun/contracts-solana-audit", "test:e2e"],
    {
      env: solanaTestEnv,
    }
  );
